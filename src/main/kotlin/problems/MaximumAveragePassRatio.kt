 ...
 * Output: 0.78333
 * Explanation:
 * You can assign the two extra students to the first class. The average pass ratio will be:
 ...
 * Output: 0.53485
 * Explanation:
 * You can assign the four extra students to the second class. The average pass ratio will be:
 * (2/4 + 7/9 + 4/5 + 2/10) / 4 = 0.53485
 *
 * Constraints:
...
class MaximumAveragePassRatio {
    ...
    fun maxAverageRatio(classes: Array<IntArray>, extraStudents: Int): Double {
        // Create max heap sorted by improvement in pass ratio when adding one student
        val maxHeap = PriorityQueue<Pair<Int, Int>> { a, b ->
            val improvementA = calculateImprovement(a.first, a.second)
            val improvementB = calculateImprovement(b.first, b.second)
            improvementB.compareTo(improvementA)
        }
        
        // Add all classes to heap
        for (c in classes) {
            maxHeap.offer(Pair(c[0], c[1]))
        }
        
        // Assign extra students one by one to maximize improvement
        repeat(extraStudents) {
            val (pass, total) = maxHeap.poll()
            // Add student to class (increases both pass and total by 1)
            maxHeap.offer(Pair(pass + 1, total + 1))
        }
    ...
    /**
     * Calculates improvement in pass ratio when adding one student
     * Delta = (pass + 1)/(total + 1) - pass/total
     * To avoid floating-point precision issues, we multiply both sides by total*(total+1)
     */
    private fun calculateImprovement(pass: Int, total: Int): Double {
